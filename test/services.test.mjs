...
test('collection speech rocket routes reject missing token and never trust client score', () => {
    ...
    assert.match(htmlSource, /if \(recognition\.lang !== 'de-DE'\) recognition\.lang = 'de-DE'/);
    assert.match(htmlSource, /maxAlternatives' in recognition/);
    ...
    assert.equal(GAME_MAX_ATTEMPTS, 2);
