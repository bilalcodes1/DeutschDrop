...
test('collection underwater worm routes reject missing token and never trust client score', () => {
    ...
    assert.match(htmlSource, /\.visual-row/);
    assert.match(htmlSource, /flex-direction: row/);
    assert.match(htmlSource, /flex-wrap: nowrap/);
    assert.match(htmlSource, /visualMarkup\(question\.visualEmoji\)/);
