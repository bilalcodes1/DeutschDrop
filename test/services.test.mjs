...
test('collection game sessions are token scoped and collection permission checked', () => {
    ...
    assert.equal(GAME_QUESTION_LIMIT, 100);
...
test('collection underwater worm routes reject missing token and never trust client score', () => {
    ...
    assert.match(routeSource, /url\.pathname === '\/game\/api\/restart'/);
    assert.match(routeSource, /restartGameSession\(env\.DB, body\.token/);
    assert.match(routeSource, /gameUrl: `\/game\?token=\$\{encodeURIComponent\(next\.token\)\}&v=/);
    ...
    assert.match(routeSource, /confidence/);
    assert.match(routeSource, /interimTranscript/);
    ...
    assert.match(htmlSource, /api\('\/game\/api\/restart'/);
    assert.match(htmlSource, /let token =/);
    assert.match(htmlSource, /history\.replaceState\(null, '', next\.gameUrl/);
    assert.match(htmlSource, /restartBusy/);
    ...
    assert.match(htmlSource, /water-current/);
    assert.match(htmlSource, /fish-swim/);
    assert.match(htmlSource, /\.meaning-bubble/);
    assert.match(htmlSource, /\.meaning-text/);
    assert.match(htmlSource, /bubblePositionStyle\(question\.questionIndex\)/);
    assert.match(htmlSource, /randomBubblePosition/);
    assert.match(htmlSource, /--bubble-left/);
    assert.match(htmlSource, /--bubble-top/);
    assert.doesNotMatch(htmlSource, /visualMarkup\(question\.visualEmoji\)/);
    assert.match(htmlSource, /\.worm/);
    assert.match(htmlSource, /\.worm-body/);
    assert.match(htmlSource, /\.worm-chomp/);
    assert.match(htmlSource, /\.worm-grow/);
    assert.match(htmlSource, /\.worm-chasing/);
    assert.match(htmlSource, /\.worm-eating/);
    assert.match(htmlSource, /\.worm-celebrate/);
    assert.match(htmlSource, /\.worm-segment/);
    assert.match(htmlSource, /\.worm-tail/);
    assert.match(htmlSource, /\.worm-eye/);
    assert.match(htmlSource, /\.worm-mouth/);
    assert.match(htmlSource, /segment-wiggle/);
    assert.match(htmlSource, /tail-wave/);
    assert.match(htmlSource, /worm-blink/);
    assert.match(htmlSource, /worm-breathe/);
    assert.match(htmlSource, /\.worm-retreat/);
    assert.match(htmlSource, /\.worm-confused/);
    assert.match(htmlSource, /\.bubble-spawn/);
    assert.match(htmlSource, /\.bubble-bite/);
    assert.match(htmlSource, /\.meaning-bubble\.final-pop/);
    assert.match(htmlSource, /final-bubble-pop/);
    assert.match(htmlSource, /\.bubble-pop/);
    assert.match(htmlSource, /\.bubble-shake/);
    assert.match(htmlSource, /\.success-sparkle/);
    assert.match(htmlSource, /\.win-burst/);
    assert.match(htmlSource, /win-bubble-rise/);
    assert.match(htmlSource, /\.score-pulse/);
    assert.match(htmlSource, /\.attempts-hit/);
    ...
    assert.match(htmlSource, /window\.AudioContext \|\| window\.webkitAudioContext/);
    assert.match(htmlSource, /startAmbientWater/);
    assert.match(htmlSource, /playSound\('listen'\)/);
    assert.match(htmlSource, /playSound\('correct'\)/);
    assert.match(htmlSource, /playSound\('wrong'\)/);
    assert.match(htmlSource, /playSound\('gameover'\)/);
    assert.match(htmlSource, /playSound\('win'\)/);
    assert.match(htmlSource, /soundEnabled/);
    assert.match(htmlSource, /🎙 تفعيل المايكروفون وابدأ/);
    assert.match(htmlSource, /microphoneEnabled = true/);
    assert.match(htmlSource, /scheduleAutoListen/);
    assert.match(htmlSource, /autoListenTimer/);
    ...
    assert.match(htmlSource, /maxAlternatives' in activeRecognition/);
    assert.match(htmlSource, /continuous = false/);
    assert.match(htmlSource, /interimResults = true/);
    assert.match(htmlSource, /latestInterimTranscript/);
    assert.match(htmlSource, /latestConfidence/);
    assert.match(htmlSource, /confidence/);
    assert.match(htmlSource, /interimTranscript/);
    assert.match(htmlSource, /not-allowed/);
    assert.match(htmlSource, /showMicrophoneRecovery/);
    assert.match(htmlSource, /visibilitychange/);
    assert.match(htmlSource, /pagehide/);
    assert.match(htmlSource, /beforeunload/);
    assert.match(htmlSource, /pageshow/);
    assert.match(htmlSource, /stopListening\(\)/);
    assert.match(htmlSource, /clearTimers\(\)/);
    assert.match(htmlSource, /abort/);
    assert.match(htmlSource, /sendBeacon/);
    assert.match(htmlSource, /keepalive: true/);
    assert.match(htmlSource, /finishOnExit/);
    assert.match(htmlSource, /finishPayload\(reason = 'exit'\)/);
    assert.match(htmlSource, /finishOnExit\(false, 'pagehide'\)/);
    assert.match(htmlSource, /finishOnExit\(false, 'visibility_hidden'\)/);
    assert.match(htmlSource, /finishOnExit\(false, 'beforeunload'\)/);
    assert.match(htmlSource, /تم حفظ تقدمك/);
    assert.match(htmlSource, /ربحت ' \+ Number\(xpGained \|\| 0\) \+ ' XP/);
    assert.match(htmlSource, /isListening/);
    assert.match(htmlSource, /isChecking/);
    assert.match(htmlSource, /isGameOver/);
    assert.match(htmlSource, /isRestarting/);
    assert.match(htmlSource, /currentQuestionIndex/);
    assert.match(htmlSource, /activeTimerId/);
    assert.match(htmlSource, /wormPosition/);
    assert.match(htmlSource, /wormDirection/);
    assert.match(htmlSource, /wormMoveTimer/);
    assert.match(htmlSource, /startWormIdleMovement/);
    assert.match(htmlSource, /moveWormToBubble\(finalWin\)/);
    assert.match(htmlSource, /stopWormMovement/);
    ...
    assert.match(htmlSource, /--play-top/);
    assert.match(htmlSource, /--play-bottom/);
    assert.match(htmlSource, /100dvh/);
    assert.match(htmlSource, /clamp\(/);
    assert.match(htmlSource, /prefers-reduced-motion/);
    assert.match(htmlSource, /summary-grid/);
    assert.match(htmlSource, /الكلمات المكتملة/);
    assert.match(htmlSource, /النقاط المكتسبة/);
    assert.match(htmlSource, /XP المكتسب/);
    assert.match(htmlSource, /ممتاز! أكملت المجموعة/);
    assert.match(htmlSource, /الدودة أكلت كل الفقاعات بنجاح/);
    assert.match(htmlSource, /formatDuration\(state\.durationMs\)/);
    ...
    assert.match(htmlSource, /utterance\.pitch = 1/);
    assert.match(htmlSource, /utterance\.volume = 1/);
    assert.match(htmlSource, /getGermanVoice/);
    assert.match(htmlSource, /startsWith\('de-de'\)/);
    assert.match(htmlSource, /startsWith\('de'\)/);
    ...
    assert.match(serviceSource, /GAME_NO_SPEECH_GRACE_COUNT/);
    assert.match(serviceSource, /noSpeechCount/);
    assert.match(serviceSource, /confidence/);
    assert.match(serviceSource, /interimTranscript/);
    assert.match(serviceSource, /tryAgain: true/);
    assert.match(serviceSource, /attemptsLeft/);
    assert.match(serviceSource, /answerReason/);
    assert.match(serviceSource, /totalWords/);
    assert.match(serviceSource, /completedWords/);
    assert.match(serviceSource, /failedAttempts/);
    assert.match(serviceSource, /attemptsByWord/);
    assert.match(serviceSource, /timeLimitSeconds/);
    assert.match(serviceSource, /durationMs/);
    assert.match(serviceSource, /isChallenge/);
    assert.match(serviceSource, /gameWon/);
    assert.match(serviceSource, /normalizeSpeechTranscript/);
    assert.match(serviceSource, /removeGermanArticle/);
    ...
    assert.equal(GAME_UI_VERSION, 'underwater-worm-v2');
    ...
    assert.equal(isAcceptedGermanAnswer('die enten', 'die Ente'), true);
    assert.equal(isAcceptedGermanAnswer('Tiger', 'der Tiger'), true);
    assert.equal(isAcceptedGermanAnswer('das Haus', 'das Haus'), true);
    assert.equal(isAcceptedGermanAnswer('haus', 'das Haus'), true);
    assert.equal(isAcceptedGermanAnswer('duck', 'die Ente'), false);
    assert.equal(isAcceptedGermanAnswer('بطة', 'die Ente'), false);
...
test('collection game visual cache remains safe but is not required to start', () => {
    ...
    assert.deepEqual(resolveEmojiVisual('Ente', 'بطة').value, '🦆');
    assert.deepEqual(resolveEmojiVisual('Tiger', 'نمر').value, '🐯');
    assert.deepEqual(resolveEmojiVisual('schreiben', 'كتابة').value, '✍️');
    assert.notEqual(resolveEmojiVisual('Unbekannteswort', 'شيء غير معروف').value, '');
    assert.equal(validateManualVisual('😀')?.source, 'manual');
    assert.equal(validateManualVisual('🏫📚')?.source, 'manual');
    assert.equal(validateManualVisual('https://example.com/image.png'), null);
    ...
    assert.equal(validateManualVisual('duck'), null);
    assert.equal(validateManualVisual('صورة'), null);
...
test('collection game finish awards capped XP once through addXp', () => {
    ...
    assert.match(serviceSource, /if \(isTerminal && session\.xp_awarded === 0\)[\s\S]*await finishGameSession\(db, token\)/);
    assert.match(serviceSource, /else if \(!isTerminal && session\.xp_awarded === 0\)[\s\S]*xpGained = 0/);
...
test('game challenge finish stores results and uses faster tie breaker', () => {
    ...
    assert.match(serviceSource, /بانتظار نتيجة الطرف الآخر/);
