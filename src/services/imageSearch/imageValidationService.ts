...
export function detectSupportedImageMime(bytes: Uint8Array, headerMime?: string | null, filePath?: string | null): string {
    if (bytes.byteLength === 0) throw new Error('empty_image_body');
    const signatureMime = detectMimeFromSignature(bytes);
    if (!signatureMime) throw new Error('unsupported_image_signature');

    const normalizedHeader = normalizeOptionalImageMime(headerMime);
    const extensionMime = mimeFromPathExtension(filePath);
    if (normalizedHeader && normalizedHeader !== signatureMime) {
        throw new Error('image_mime_mismatch');
    }
    if (extensionMime && extensionMime !== signatureMime) {
        throw new Error('image_mime_mismatch');
    }
...
function normalizeOptionalImageMime(mimeType?: string | null): string | null {
    const clean = String(mimeType ?? '').split(';')[0].trim().toLowerCase();
    if (!clean || clean === 'application/octet-stream') return null;
    if (clean === 'image/jpg') return 'image/jpeg';
    if (!isAllowedImageMime(clean)) throw new Error('unsupported_image_mime');
    return clean;
}

function mimeFromPathExtension(filePath?: string | null): string | null {
